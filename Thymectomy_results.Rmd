...
```{r complications_overall, echo=FALSE, fig.height=11, fig.width=14, message=FALSE, warning=FALSE}

head <- c(paste0('Treatment vs. Control','\n','By Complication'), 'N of studies',
          paste0('Treatment','\n','events/n'), paste0('Control','\n','events/n'),
          'I^2', 'Relative Risk (95% CI)')

subhead1 <- c(paste0('Comparison of thymectomy approaches'), rep(NA, 5))
...
subhead2 <- c(paste0('All minimally invasive vs. All median sternotomy approaches'), rep(NA, 5))
...
complications <- rbind(head, NA, NA, subhead1, NA, cp2, NA, cp3, NA, cp4, NA, cp5,
                       NA, NA, subhead2, NA, part1, NA, part2, NA)
rownames(complications) <- NULL

sub1 <- c(7, 10:14, 17, 18, 21, 22, 28:32, 35:38)
complications[,1][sub1] <- paste("        ",complications[,1][sub1])

sub2 <- c(6, 9, 16, 20, 27, 34)
...
c <- rbind(NA,  NA, NA, NA, NA, NA, tcomp2_winf$c[4,], NA,
          ...
          NA, NA, NA, NA, tminst_pn$c[8,], tminst_mya$c[9,], tminst_ate$c[4,], 
          tminst_arr$c[4,], tminst_winf$c[8,], NA,
          NA, tmin2_pn$c[4,], tmin2_mya$c[4,], tmin2_arr$c[4,], tmin2_winf$c[4,], NA)
