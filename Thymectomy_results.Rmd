...
prema8 <- escalc(measure="RR",ai=events1, ci=events2, n1i=n1, n2i=n2,
                  subset=(followup==3 & 
                            comparison=="T3b  vs.  T2b" & group=="Overall"), data=pao)
...
c <- rbind(
  c("T3a  vs.  T2b",  "3 years", ma5[[15]], sum(ma5[[42]]), sum(ma5[[43]]),
    sum(ma5[[44]]),sum(ma5[[45]]), ma5[[25]], expma5[[1]], expma5[[3]], expma5[[4]],
    ma5[[24]]), 
  c("T3a  vs.  T2b",  "4 years", ma6[[15]], sum(ma6[[42]]), sum(ma6[[43]]),
    sum(ma6[[44]]),sum(ma6[[45]]), ma6[[25]], expma6[[1]], expma6[[3]], expma6[[4]],
    ma6[[24]]),
  c("T3a  vs.  T2b", "5 years", ma7[[15]], sum(ma7[[42]]), sum(ma7[[43]]),
    sum(ma7[[44]]),sum(ma7[[45]]), ma7[[25]], expma7[[1]], expma7[[3]], expma7[[4]],
    ma7[[24]]))
  

d <- rbind(
  c("T3b  vs.  T2b", "3 years", ma8[[15]], 
  ...
  c("T3b  vs.  T2b", "8 years", ma13[[15]], 
    ...
    expma13[[4]], ma13[[24]]),
  c("T3b  vs.  T2b", "9 years", ma14[[15]], sum(ma14[[42]]), sum(ma14[[43]]),
...
```{r, fig.height=7, fig.width=18, echo=FALSE, message=FALSE, warning=FALSE}
...
lessagr <- lessagr %>% add_row(followup="Extended Transsternal vs. Basic Transcervical", .before = 1)
lessagr <- lessagr %>% add_row(followup="Standard Transsternal vs. VATS extended", .before = 3)
lessagr <- lessagr %>% add_row(followup="Extended Transsternal vs. VATS extended", .before = 7)
lessagr <- lessagr %>% add_row(followup="Standard Transsternal vs. Transcervical-Subxiphoid", .before = 15)
lessagr <- lessagr %>% add_row(followup="Extended Transsternal vs. Transcervical-Subxiphoid", .before = 19)
lessagr <- lessagr %>% add_row(followup="Extended Transsternal vs. Subxiphoid Mediastinoscopy", .before = 23)
...
sub1 <- c(4, 6:8, 10:16, 18:20, 22:24, 26)
...
less <- less %>% add_row(followup="VATS extended uni vs. VATS extended bi", .before = 1)
less <- less %>% add_row(followup="VATS extended lelft vs. VATS extended right", .before = 6)
less <- less %>% add_row(followup="VATS extended with robotic  vs. VATS extended", .before = 9)
